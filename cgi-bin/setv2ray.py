...
if form.has_key('mainport'):
    sendmainport = form['mainport'].value
else:
    exit()

if form.has_key('localport'):
    sendlocalport = form['localport'].value
else:
    exit()

if form.has_key('transport'):
    sendtransport = form['transport'].value
else:
    exit()

if form.has_key('mux'):
    sendmux = form['mux'].value
else:
    exit()
...
<h4 style="color: rgb(193, 174, 90);"><strong>配置修改成功！！</strong></h4><br>
        </div>
  </div>
...
v2rayfunctions.genjson(sendmainport,sendtransport,sendtype,sendmux,sendlocalport)
v2rayfunctions.genclient()
